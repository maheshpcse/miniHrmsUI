...
export class AdminDashboardComponent implements OnInit, OnDestroy {
  ...
  error = '';
  data: any = {};
  name = sessionStorage.getItem('firstName') || 'there';
  ...
  load() {
    this.loading = true;
    this.error = '';
    this.request = this.api.get('/dashboard').subscribe(
      (data) => {
        this.data = data;
        this.loading = false;
      },
  ...
  get stats() {
    return [
      ...
      },
      {
        ...
        icon: 'heart',
        note: 'Ready to do great things',
      },
      {
        label: 'Pending requests',
        value: this.data.requests,
        icon: 'request',
      ...
      },
      {
        ...
        icon: 'people',
        note: 'Inactive employee records',
      },
    ];
  }
  initials(row: any) {
