...
export class ViewEmployeeComponent implements OnInit {
    ...
    getEmployeeTabsData(tab?: any) {
        if (tab == 0) this.employeeInfo = this.setNoDataToEmployee(this.empData['employeeInfo']);
            ...
            this.empOnboardingInfo = this.setNoDataToEmployee(this.empData['empOnboardingInfo']);
            for (let item of this.empData['empProcessInfo']) {
                let newItem: any = this.setNoDataToEmployee(item);
                this.empProcessInfo.push(newItem);
            }
        }
